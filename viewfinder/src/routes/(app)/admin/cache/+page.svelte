	...
	<div class="cache-container">
		...
		<div class="metrics-grid">
			...
			<div class="metric-card">
				...
				<span class="card-desc">Efficiency of cache serving requests without re-processing</span>
			</div>
		</div>
		...
		<div class="details-section">
			...
			<div class="visualization-card">
				...
				<div class="efficiency-bar-container">
					<div class="bar-fill hits" style="width: {cacheStatus.hit_ratio * 100}%" title="Hits: {(cacheStatus.hit_ratio * 100).toFixed(2)}%"></div>
					<div class="bar-fill misses" style="width: {(1 - cacheStatus.hit_ratio) * 100}%" title="Misses: {((1 - cacheStatus.hit_ratio) * 100).toFixed(2)}%"></div>
				</div>
				...
				<div class="efficiency-status" class:healthy={cacheStatus.hit_ratio >= 0.8} class:warning={cacheStatus.hit_ratio < 0.8 && cacheStatus.hit_ratio >= 0.5} class:critical={cacheStatus.hit_ratio < 0.5}>
					<MaterialIcon iconName={cacheStatus.hit_ratio >= 0.8 ? "check_circle" : (cacheStatus.hit_ratio >= 0.5 ? "warning" : "error")} />
					<span>
			...
			<div class="advisory-card">
				...
				<div class="advisory-content">
					<span>
						...
						or format upgrades, the original image is left completely unmodified. The processed output (e.g. webp preview) 
						is cached here for subsequent requests.
					</span>
					<span>
						<b>Is it safe to clear?</b> Yes. Clearing the cache will instantly free up disk space. 
						Viz will automatically re-generate any requested thumbnails or previews on-the-fly as users navigate the platform, 
						though the first load for each size may experience minor latency.
					</span>
				</div>
			</div>
		</div>
...
<style lang="scss">
	...
	.visualization-card, .advisory-card {
		background-color: var(--viz-95);
		border: var(--viz-border-thin);
		border-radius: var(--viz-border-radius-md);
