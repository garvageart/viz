...
test.describe("View Switching Functionality", () => {
    test("should switch views on /photos", async ({ page }) => {
        ...
        // 2. Open Display dropdown via ID selector
    ...
    test("should switch views on /search", async ({ page }) => {
        ...
        // Find the Display dropdown within the images section via ID selector
        const imageDisplayDropdown = imagesSection.locator("#search-image-display-dropdown").first();
    ...
    test("should switch views on /collections/[uid]", async ({ page }) => {
        ...
        await displayDropdown.click();
        await page
            .locator('#display-custom, #display-grid, [id="display-custom"], [id="display-grid"]')
            .first()
            .click();
        await expect(page.locator(".viz-photo-grid-container, .viz-asset-grid-container")).toBeVisible();
    });
});
