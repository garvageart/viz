...
test.describe('Material Icon E2E Tests', () => {
	...
	test('should render icons inside Create Collection modal as SVGs and not fallback codepoint text', async ({ page }) => {
	...
	test('should render icons inside upload confirmation modal and upload panel as SVGs and not fallback codepoint text', async ({ page }) => {
		// Go to photos page
		await page.goto('/photos');
		await page.waitForLoadState('networkidle');
		await expect(page.locator('.viz-view-container')).toBeVisible({ timeout: 20000 });
		...
		const modal = page.locator('.modal-inner', { hasText: 'How would you like to upload them?' });
		...
		await expect(page.locator('.viz-toast-success').filter({ hasText: 'Successfully uploaded' })).toBeVisible({ timeout: 30000 });
	});

	test.describe('Unauthenticated Auth Pages', () => {
		test.use({ storageState: { cookies: [], origins: [] } });

		test('should render auth login and register page icons as SVGs and not fallback codepoint text', async ({ page }) => {
