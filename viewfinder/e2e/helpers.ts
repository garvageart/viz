...
export async function cleanupTestCollections(request: APIRequestContext) {
    try {
        ...
        if (!response.ok()) {
            console.error(
                `Failed to list collections for cleanup: ${response.status()} ${response.statusText()}`
            );
            return;
        }
        ...
        for (const collection of collections) {
            if (collection.name.startsWith("E2E-")) {
                console.log(
                    `Cleaning up leftover E2E collection: ${collection.name} (${collection.uid})`
                );
                const deleteRes = await request.delete(`/api/collections/${collection.uid}`);
                if (!deleteRes.ok()) {
                    console.error(
                        `Failed to delete collection ${collection.uid}: ${deleteRes.status()}`
                    );
                }
            }
        }
