...
test.describe('Advanced Search & Image Filtering', () => {
    ...
    test('should toggle EXIF focal length slider filters and reset', async ({ page }) => {
        // Locate EXIF section
        const exifHeader = page.locator('.filter-section button.section-header', { hasText: 'EXIF' });
    ...
    test('should manage Checklist facets (Keywords / Cameras)', async ({ page }) => {
        ...
        const keywordsHeader = page.locator('.filter-section button.section-header', { hasText: 'Keywords' });
