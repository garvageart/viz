...
* feat(config): add trim_file_extensions option for image imports (fd24556a)
* refactor(viewfinder): extract StatusIndicator component for lightbox status messages (2aa1fef3)
* refactor(viewfinder): simplify toast notification components and calls (b5f7d112)
...
* fix(viewfinder): preserve selection anchor during range selection (63d25df3)
...
* refactor(viewfinder): standardize collection toasts and double-click target (f18ed090)
* test(e2e): remove networkidle wait in upload test (144b38ff)
* style(viewfinder): update default photo grid virtualizer gap (b4c1451b)

## [0.34.12] - 2026-09-14

* feat(viewfinder): add workspace layout json copy button to header (10622b00)
* fix(viewfinder): register dynamic collection selection scopes in filmstrip and collection routes (faeab436)
...
* feat(viewfinder): isolate scrollable content in AssetsShell so toolbar spans full container width (3b2503de)
...
* fix(ui): lower navigation progress bar z-index to chrome level (4cfc5633)
* test(fixtures): replace oversized sample images with lightweight EXIF-preserved fixtures (fb2359fb)
...
* ci(workflow): use fully qualified GitHub URL for release action (39da2faa)
...
* ci(workflow): build locally and publish images only after tests pass (6b748db2)

## [0.32.10] - 2026-08-26

* ci(workflow): tag images with :latest for commits and :stable/:v* for releases (77b57fab)
...
* ci: fix awk regex syntax for changelog release notes extraction (26fc8fed)

## [0.32.6] - 2026-08-24

* ci: replace third-party release action with universal api release step (f7215bf8)
...
* ci: add submodule checkout, remove registry cache flags, and pass explicit deps tags (f715aee0)
* fix(viewfinder): generate missing IconRemove component (4f403c73)
