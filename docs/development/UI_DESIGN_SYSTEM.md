...
- **Pill Shape** (`--viz-border-radius-pill`): `9999px` – **Strictly reserved** for interactive action buttons (`Button`) to create high-contrast focus targets.
...
4. **Avoid Fallback Values in Design System CSS Variables**: Do not provide fallback values when referencing core design system tokens (e.g. use `var(--viz-spacing-md)` or `var(--viz-surface-card)` instead of `var(--viz-spacing-md, 0.75rem)`). Design tokens are guaranteed to exist at root. Fallbacks in `var(...)` should be reserved strictly for theming hooks, component-level custom properties, or intentional overrides (e.g., `var(--viz-crop-mask, rgba(0, 0, 0, 0.6))`).
