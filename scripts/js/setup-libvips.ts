...
async function installWindows() {
    ...
    try {
        ...
        error(
            "Please ensure MSYS2 is installed and this script is run from an MSYS2 terminal (MinGW 64-bit recommended)."
        );
        process.exit(1);
    }
...
function checkVersionMatch(required: string): boolean {
    try {
        const vipsVersionOutput = execSync("vips --version", {
            encoding: "utf-8"
        }).trim();
...
async function installFromSourceLinux(version: string) {
    ...
    try {
        ...
        runCommand("meson", ["setup", "build", "--prefix=/usr"], {
            cwd: sourceDir
        });
        ...
        log("Installing...");
        runCommand("sudo", ["ninja", "-C", "build", "install"], {
            cwd: sourceDir
        });
...
async function installLinux(requiredVersion: string) {
    ...
    if (hasApt) {
        ...
        try {
            const policy = execSync(`apt-cache policy libvips-dev`, {
                encoding: "utf-8"
            });
...
async function verify(requiredVersion: string) {
    ...
    try {
        const libs = execSync("pkg-config --cflags --libs vips", {
            encoding: "utf-8"
        }).trim();
        if (libs) {
