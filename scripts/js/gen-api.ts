...
if (options.installTools) {
    ...
    if (existsSync(vizDir) && commandExists("pnpm")) {
        ...
        console.warn(
            "pnpm not found. Skipping TS generator install; assuming it's available in PATH."
        );
    }
}
...
if (!typesSuccess) {
    console.warn(
        "oapi-codegen types generation into pkg/openapi failed; pkg/openapi will not contain types"
    );
} else {
...
if (!typesSuccess && !existsSync(dtoOutPath)) {
    console.warn(
        "No types available in pkg/openapi OR internal/dto; ensure oapi-codegen types run succeeded earlier."
    );
}
    ...
    if (existsSync(pkgJsonPath)) {
        if (commandExists("pnpm")) {
            ...
            if (!tsGenSuccess) {
                ...
                run(
                    "npx",
                    ["oazapfts", specPath, join("src", "lib", "api", "client.gen.ts")],
                    vizDir
                );
            }
        } else {
...
`;
            ...
            for (const [value, label] of Object.entries(scopesMap)) {
                ...
                scopeKeysGo.push(
                    `\t// ${goKey} grants permission for ${value}.\n\t${goKey} Scope = "${value}"`
                );
                scopeItemsGo.push(`\t{Value: ${goKey}, Label: "${label}"}`);
            }

            const goGenContent = `package auth
