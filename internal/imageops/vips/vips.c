...
int vipsgen_Oklab2Oklch(VipsImage* in, VipsImage** out) {
    return vips_Oklab2Oklch(in, out, NULL);
}

int vipsgen_Oklab2XYZ(VipsImage* in, VipsImage** out) {
    return vips_Oklab2XYZ(in, out, NULL);
}

int vipsgen_Oklch2Oklab(VipsImage* in, VipsImage** out) {
    return vips_Oklch2Oklab(in, out, NULL);
}
...
int vipsgen_XYZ2Oklab(VipsImage* in, VipsImage** out) {
    return vips_XYZ2Oklab(in, out, NULL);
}
...
int vipsgen_heifsave_with_options(VipsImage* in, const char* filename, gint Q, gint bitdepth, gboolean lossless, VipsForeignHeifCompression compression, gint effort, VipsForeignSubsample subsample_mode, VipsForeignHeifEncoder encoder, const char* tune, VipsForeignKeep keep, double* background, int background_n, gint page_height, const char* profile) {
    ...
    if (
        ...
        vipsgen_set_string(operation, "tune", tune) ||
        vipsgen_set_int(operation, "keep", keep) ||
        vipsgen_set_array_double(operation, "background", background_array) ||
        vipsgen_set_int(operation, "page_height", page_height) ||
        vipsgen_set_string(operation, "profile", profile)
...
int vipsgen_jxlsave_with_options(VipsImage* in, const char* filename, gint tier, double distance, gint effort, gboolean lossless, gint Q, gint bitdepth, VipsForeignKeep keep, double* background, int background_n, gint page_height, const char* profile) {
...
int vipsgen_jxlsave_buffer_with_options(VipsImage* in, void** buf, size_t* len, gint tier, double distance, gint effort, gboolean lossless, gint Q, gint bitdepth, VipsForeignKeep keep, double* background, int background_n, gint page_height, const char* profile) {
...
int vipsgen_jxlsave_target_with_options(VipsImage* in, VipsTargetCustom* target, gint tier, double distance, gint effort, gboolean lossless, gint Q, gint bitdepth, VipsForeignKeep keep, double* background, int background_n, gint page_height, const char* profile) {
...
int vipsgen_magickload_source(VipsSourceCustom* source, VipsImage** out) {
    return vips_magickload_source((VipsSource*) source, out, NULL);
}

int vipsgen_magickload_source_with_options(VipsSourceCustom* source, VipsImage** out, const char* density, gint page, gint n, gboolean memory, VipsAccess access, VipsFailOn fail_on, gboolean revalidate) {
    VipsOperation *operation = vips_operation_new("magickload_source");
    if (!operation) return 1;
    if (
        vips_object_set(VIPS_OBJECT(operation), "source", (VipsSource*)source, NULL) ||
        vipsgen_set_string(operation, "density", density) ||
...
int vipsgen_pdfload_with_options(const char* filename, VipsImage** out, gint page, gint n, double dpi, double scale, double* background, int background_n, const char* password, VipsForeignPdfPageBox page_box, gboolean memory, VipsAccess access, VipsFailOn fail_on, gboolean revalidate) {
    ...
    if (
        ...
        vipsgen_set_int(operation, "page_box", page_box) ||
        vipsgen_set_bool(operation, "memory", memory) ||
        vipsgen_set_int(operation, "access", access) ||
        vipsgen_set_int(operation, "fail_on", fail_on) ||
        vipsgen_set_bool(operation, "revalidate", revalidate)
...
int vipsgen_pdfload_buffer_with_options(void* buf, size_t len, VipsImage** out, gint page, gint n, double dpi, double scale, double* background, int background_n, const char* password, VipsForeignPdfPageBox page_box, gboolean memory, VipsAccess access, VipsFailOn fail_on, gboolean revalidate) {
...
int vipsgen_pdfload_source_with_options(VipsSourceCustom* source, VipsImage** out, gint page, gint n, double dpi, double scale, double* background, int background_n, const char* password, VipsForeignPdfPageBox page_box, gboolean memory, VipsAccess access, VipsFailOn fail_on, gboolean revalidate) {
...
int vipsgen_system_with_options(const char* cmd_format, VipsImage** in, int in_n, const char* in_format, const char* out_format, gboolean cache) {
    ...
    if (
        ...
        vipsgen_set_bool(operation, "cache", cache)
    ) {
        g_object_unref(operation);
        if (in_array != NULL) { vips_area_unref(VIPS_AREA(in_array)); }
        return 1;
...
int vipsgen_uhdr2scRGB(VipsImage* in, VipsImage** out) {
    return vips_uhdr2scRGB(in, out, NULL);
}

int vipsgen_uhdrload(const char* filename, VipsImage** out) {
    return vips_uhdrload(filename, out, NULL);
}

int vipsgen_uhdrload_with_options(const char* filename, VipsImage** out, gint shrink, gboolean memory, VipsAccess access, VipsFailOn fail_on, gboolean revalidate) {
    VipsOperation *operation = vips_operation_new("uhdrload");
    if (!operation) return 1;
    if (
        vips_object_set(VIPS_OBJECT(operation), "filename", filename, NULL) ||
        vipsgen_set_int(operation, "shrink", shrink) ||
...
int vipsgen_uhdrload_buffer(void* buf, size_t len, VipsImage** out) {
    return vips_uhdrload_buffer(buf, len, out, NULL);
}

int vipsgen_uhdrload_buffer_with_options(void* buf, size_t len, VipsImage** out, gint shrink, gboolean memory, VipsAccess access, VipsFailOn fail_on, gboolean revalidate) {
    VipsOperation *operation = vips_operation_new("uhdrload_buffer");
    if (!operation) return 1;
    VipsBlob *blob = vips_blob_new(NULL, buf, len);
    if (!blob) { g_object_unref(operation); return 1; }
    if (
...
int vipsgen_uhdrload_source(VipsSourceCustom* source, VipsImage** out) {
    return vips_uhdrload_source((VipsSource*) source, out, NULL);
}

int vipsgen_uhdrload_source_with_options(VipsSourceCustom* source, VipsImage** out, gint shrink, gboolean memory, VipsAccess access, VipsFailOn fail_on, gboolean revalidate) {
    VipsOperation *operation = vips_operation_new("uhdrload_source");
    if (!operation) return 1;
    if (
        vips_object_set(VIPS_OBJECT(operation), "source", (VipsSource*)source, NULL) ||
        vipsgen_set_int(operation, "shrink", shrink) ||
...
int vipsgen_uhdrsave(VipsImage* in, const char* filename) {
    return vips_uhdrsave(in, filename, NULL);
}

int vipsgen_uhdrsave_with_options(VipsImage* in, const char* filename, gint Q, VipsForeignKeep keep, double* background, int background_n, gint page_height, const char* profile) {
    VipsOperation *operation = vips_operation_new("uhdrsave");
    if (!operation) return 1;
    VipsArrayDouble *background_array = NULL;
    if (background != NULL && background_n > 0) { background_array = vips_array_double_new(background, background_n); }
    if (
...
int vipsgen_uhdrsave_buffer(VipsImage* in, void** buf, size_t* len) {
    return vips_uhdrsave_buffer(in, buf, len, NULL);
}

int vipsgen_uhdrsave_buffer_with_options(VipsImage* in, void** buf, size_t* len, gint Q, VipsForeignKeep keep, double* background, int background_n, gint page_height, const char* profile) {
    VipsOperation *operation = vips_operation_new("uhdrsave_buffer");
    if (!operation) return 1;
    VipsArrayDouble *background_array = NULL;
    if (background != NULL && background_n > 0) { background_array = vips_array_double_new(background, background_n); }
    if (
...
int vipsgen_uhdrsave_target(VipsImage* in, VipsTargetCustom* target) {
    return vips_uhdrsave_target(in, (VipsTarget*) target, NULL);
}

int vipsgen_uhdrsave_target_with_options(VipsImage* in, VipsTargetCustom* target, gint Q, VipsForeignKeep keep, double* background, int background_n, gint page_height, const char* profile) {
    VipsOperation *operation = vips_operation_new("uhdrsave_target");
    if (!operation) return 1;
    VipsArrayDouble *background_array = NULL;
    if (background != NULL && background_n > 0) { background_array = vips_array_double_new(background, background_n); }
    if (
...
int vipsgen_webpsave_with_options(VipsImage* in, const char* filename, gint Q, gboolean lossless, gboolean exact, VipsForeignWebpPreset preset, gboolean smart_subsample, gboolean near_lossless, gint alpha_q, gboolean min_size, gint kmin, gint kmax, gint effort, gint target_size, gboolean mixed, gboolean smart_deblock, gint passes, VipsForeignKeep keep, double* background, int background_n, gint page_height, const char* profile) {
    ...
    if (
        ...
        vipsgen_set_int(operation, "Q", Q) ||
        vipsgen_set_bool(operation, "lossless", lossless) ||
        vipsgen_set_bool(operation, "exact", exact) ||
