...
func DefaultMagickloadSourceOptions() *MagickloadSourceOptions {
	return &MagickloadSourceOptions{
		N: 1,
	}
}
...
func NewMagickloadSource(source *Source, options *MagickloadSourceOptions) (*Image, error) {
	Startup(nil)
	if options != nil {
		vipsImage, err := vipsgenMagickloadSourceWithOptions(source.src, options.Density, options.Page, options.N, options.Memory, options.Access, options.FailOn, options.Revalidate)
		if err != nil {
			return nil, err
		}
		return newImageRef(vipsImage, ImageTypeMagick, nil), nil
	}
	vipsImage, err := vipsgenMagickloadSource(source.src)
	if err != nil {
		return nil, err
	}
	return newImageRef(vipsImage, ImageTypeMagick, nil), nil
...
func DefaultPdfloadOptions() *PdfloadOptions {
	return &PdfloadOptions{
		...
		Dpi: 72,
		Scale: 1,
		PageBox: PdfPageBox(1),
	}
}

// NewPdfload vips_pdfload load PDF from file (poppler)
//
// The filename specifies filename to load from.
func NewPdfload(filename string, options *PdfloadOptions) (*Image, error) {
	Startup(nil)
	if options != nil {
		vipsImage, err := vipsgenPdfloadWithOptions(filename, options.Page, options.N, options.Dpi, options.Scale, options.Background, options.Password, options.PageBox, options.Memory, options.Access, options.FailOn, options.Revalidate)
		if err != nil {
			return nil, err
		}
...
// NewPdfloadBuffer vips_pdfload_buffer load PDF from buffer (poppler)
func NewPdfloadBuffer(buf []byte, options *PdfloadBufferOptions) (*Image, error) {
	...
	if options != nil {
		vipsImage, err := vipsgenPdfloadBufferWithOptions(buf, options.Page, options.N, options.Dpi, options.Scale, options.Background, options.Password, options.PageBox, options.Memory, options.Access, options.FailOn, options.Revalidate)
		if err != nil {
			return nil, err
		}
...
// NewPdfloadSource vips_pdfload_source load PDF from source (poppler)
//
// The source specifies source to load from.
func NewPdfloadSource(source *Source, options *PdfloadSourceOptions) (*Image, error) {
	Startup(nil)
	if options != nil {
		vipsImage, err := vipsgenPdfloadSourceWithOptions(source.src, options.Page, options.N, options.Dpi, options.Scale, options.Background, options.Password, options.PageBox, options.Memory, options.Access, options.FailOn, options.Revalidate)
		if err != nil {
			return nil, err
		}
		return newImageRef(vipsImage, ImageTypePdf, nil), nil
...
func NewSystem(cmdFormat string, options *SystemOptions) (*Image, error) {
	Startup(nil)
	if options != nil {
		vipsImage, err := vipsgenSystemWithOptions(cmdFormat, convertImagesToVipsImages(options.In), options.InFormat, options.OutFormat, options.Cache)
		if err != nil {
			return nil, err
		}
		return newImageRef(vipsImage, ImageTypeUnknown, nil), nil
...
// UhdrloadOptions optional arguments for vips_uhdrload
...
// DefaultUhdrloadOptions creates default value for vips_uhdrload optional arguments
func DefaultUhdrloadOptions() *UhdrloadOptions {
	return &UhdrloadOptions{
		Shrink: 1,
	}
}

// NewUhdrload vips_uhdrload load a UHDR image
//
// The filename specifies filename to load from.
func NewUhdrload(filename string, options *UhdrloadOptions) (*Image, error) {
	Startup(nil)
	if options != nil {
		vipsImage, err := vipsgenUhdrloadWithOptions(filename, options.Shrink, options.Memory, options.Access, options.FailOn, options.Revalidate)
		if err != nil {
			return nil, err
		}
		return newImageRef(vipsImage, ImageTypeUhdr, nil), nil
	}
	vipsImage, err := vipsgenUhdrload(filename)
	if err != nil {
		return nil, err
	}
	return newImageRef(vipsImage, ImageTypeUhdr, nil), nil
...
// UhdrloadBufferOptions optional arguments for vips_uhdrload_buffer
...
// DefaultUhdrloadBufferOptions creates default value for vips_uhdrload_buffer optional arguments
func DefaultUhdrloadBufferOptions() *UhdrloadBufferOptions {
	return &UhdrloadBufferOptions{
		Shrink: 1,
	}
}

// NewUhdrloadBuffer vips_uhdrload_buffer load a UHDR image
func NewUhdrloadBuffer(buf []byte, options *UhdrloadBufferOptions) (*Image, error) {
	Startup(nil)
	if len(buf) == 0 {
		return nil, fmt.Errorf("uhdrload_buffer: buffer is empty")
	}
	if options != nil {
		vipsImage, err := vipsgenUhdrloadBufferWithOptions(buf, options.Shrink, options.Memory, options.Access, options.FailOn, options.Revalidate)
		if err != nil {
			return nil, err
		}
		return newImageRef(vipsImage, ImageTypeUhdr, buf), nil
	}
	vipsImage, err := vipsgenUhdrloadBuffer(buf)
	if err != nil {
		return nil, err
	}
	return newImageRef(vipsImage, ImageTypeUhdr, buf), nil
...
// UhdrloadSourceOptions optional arguments for vips_uhdrload_source
...
// DefaultUhdrloadSourceOptions creates default value for vips_uhdrload_source optional arguments
func DefaultUhdrloadSourceOptions() *UhdrloadSourceOptions {
	return &UhdrloadSourceOptions{
		Shrink: 1,
	}
}

// NewUhdrloadSource vips_uhdrload_source load a UHDR image
//
// The source specifies source to load from.
func NewUhdrloadSource(source *Source, options *UhdrloadSourceOptions) (*Image, error) {
	Startup(nil)
	if options != nil {
		vipsImage, err := vipsgenUhdrloadSourceWithOptions(source.src, options.Shrink, options.Memory, options.Access, options.FailOn, options.Revalidate)
		if err != nil {
			return nil, err
		}
		return newImageRef(vipsImage, ImageTypeUhdr, nil), nil
	}
	vipsImage, err := vipsgenUhdrloadSource(source.src)
	if err != nil {
		return nil, err
	}
	return newImageRef(vipsImage, ImageTypeUhdr, nil), nil
...
// Oklab2Oklch vips_Oklab2Oklch transform Oklab to Oklch
func (r *Image) Oklab2Oklch() (error) {
	out, err := vipsgenOklab2Oklch(r.image)
	if err != nil {
		return err
	}
	r.setImage(out)
...
// Oklab2XYZ vips_Oklab2XYZ transform Oklab to XYZ
func (r *Image) Oklab2XYZ() (error) {
	out, err := vipsgenOklab2XYZ(r.image)
	if err != nil {
		return err
	}
	r.setImage(out)
...
// Oklch2Oklab vips_Oklch2Oklab transform Oklch to Oklab
func (r *Image) Oklch2Oklab() (error) {
	out, err := vipsgenOklch2Oklab(r.image)
	if err != nil {
		return err
	}
	r.setImage(out)
...
// XYZ2Oklab vips_XYZ2Oklab transform XYZ to Oklab
func (r *Image) XYZ2Oklab() (error) {
	out, err := vipsgenXYZ2Oklab(r.image)
	if err != nil {
		return err
	}
	r.setImage(out)
...
func (r *Image) Heifsave(filename string, options *HeifsaveOptions) (error) {
	if options != nil {
		err := vipsgenHeifsaveWithOptions(r.image, filename, options.Q, options.Bitdepth, options.Lossless, options.Compression, options.Effort, options.SubsampleMode, options.Encoder, options.Tune, options.Keep, options.Background, options.PageHeight, options.Profile)
		if err != nil {
			return err
		}
		return nil
...
func (r *Image) HeifsaveBuffer(options *HeifsaveBufferOptions) ([]byte, error) {
	if options != nil {
		buf, err := vipsgenHeifsaveBufferWithOptions(r.image, options.Q, options.Bitdepth, options.Lossless, options.Compression, options.Effort, options.SubsampleMode, options.Encoder, options.Tune, options.Keep, options.Background, options.PageHeight, options.Profile)
		if err != nil {
			return nil, err
		}
		return buf, nil
...
// Jpegsave vips_jpegsave save as jpeg
//
// The filename specifies filename to save to.
...
// JpegsaveBuffer vips_jpegsave_buffer save as jpeg
...
// JpegsaveTarget vips_jpegsave_target save as jpeg
//
// The target specifies target to save to.
...
func (r *Image) Jxlsave(filename string, options *JxlsaveOptions) (error) {
	if options != nil {
		err := vipsgenJxlsaveWithOptions(r.image, filename, options.Tier, options.Distance, options.Effort, options.Lossless, options.Q, options.Bitdepth, options.Keep, options.Background, options.PageHeight, options.Profile)
		if err != nil {
			return err
		}
		return nil
...
func (r *Image) JxlsaveBuffer(options *JxlsaveBufferOptions) ([]byte, error) {
	if options != nil {
		buf, err := vipsgenJxlsaveBufferWithOptions(r.image, options.Tier, options.Distance, options.Effort, options.Lossless, options.Q, options.Bitdepth, options.Keep, options.Background, options.PageHeight, options.Profile)
		if err != nil {
			return nil, err
		}
		return buf, nil
...
// Uhdr2scRGB vips_uhdr2scRGB transform uhdr to scRGB
func (r *Image) Uhdr2scRGB() (error) {
	out, err := vipsgenUhdr2scRGB(r.image)
	if err != nil {
		return err
	}
	r.setImage(out)
...
// UhdrsaveOptions optional arguments for vips_uhdrsave
...
// DefaultUhdrsaveOptions creates default value for vips_uhdrsave optional arguments
func DefaultUhdrsaveOptions() *UhdrsaveOptions {
	return &UhdrsaveOptions{
		Q: 75,
	}
}

// Uhdrsave vips_uhdrsave save image in UltraHDR format
//
// The filename specifies filename to save to.
func (r *Image) Uhdrsave(filename string, options *UhdrsaveOptions) (error) {
	if options != nil {
		err := vipsgenUhdrsaveWithOptions(r.image, filename, options.Q, options.Keep, options.Background, options.PageHeight, options.Profile)
		if err != nil {
			return err
		}
		return nil
	}
	err := vipsgenUhdrsave(r.image, filename)
	if err != nil {
		return err
	}
	return nil
...
// UhdrsaveBufferOptions optional arguments for vips_uhdrsave_buffer
...
// DefaultUhdrsaveBufferOptions creates default value for vips_uhdrsave_buffer optional arguments
func DefaultUhdrsaveBufferOptions() *UhdrsaveBufferOptions {
	return &UhdrsaveBufferOptions{
		Q: 75,
	}
}

// UhdrsaveBuffer vips_uhdrsave_buffer save image in UltraHDR format
func (r *Image) UhdrsaveBuffer(options *UhdrsaveBufferOptions) ([]byte, error) {
	if options != nil {
		buf, err := vipsgenUhdrsaveBufferWithOptions(r.image, options.Q, options.Keep, options.Background, options.PageHeight, options.Profile)
		if err != nil {
			return nil, err
		}
		return buf, nil
	}
	buf, err := vipsgenUhdrsaveBuffer(r.image)
	if err != nil {
		return nil, err
	}
	return buf, nil
...
// UhdrsaveTargetOptions optional arguments for vips_uhdrsave_target
...
// DefaultUhdrsaveTargetOptions creates default value for vips_uhdrsave_target optional arguments
func DefaultUhdrsaveTargetOptions() *UhdrsaveTargetOptions {
	return &UhdrsaveTargetOptions{
		Q: 75,
	}
}

// UhdrsaveTarget vips_uhdrsave_target save image in UltraHDR format
//
// The target specifies target to save to.
func (r *Image) UhdrsaveTarget(target *Target, options *UhdrsaveTargetOptions) (error) {
	if options != nil {
		err := vipsgenUhdrsaveTargetWithOptions(r.image, target.target, options.Q, options.Keep, options.Background, options.PageHeight, options.Profile)
		if err != nil {
			return err
		}
		return nil
	}
	err := vipsgenUhdrsaveTarget(r.image, target.target)
	if err != nil {
		return err
	}
	return nil
...
func (r *Image) Webpsave(filename string, options *WebpsaveOptions) (error) {
	if options != nil {
		err := vipsgenWebpsaveWithOptions(r.image, filename, options.Q, options.Lossless, options.Exact, options.Preset, options.SmartSubsample, options.NearLossless, options.AlphaQ, options.MinSize, options.Kmin, options.Kmax, options.Effort, options.TargetSize, options.Mixed, options.SmartDeblock, options.Passes, options.Keep, options.Background, options.PageHeight, options.Profile)
		if err != nil {
			return err
		}
		return nil
